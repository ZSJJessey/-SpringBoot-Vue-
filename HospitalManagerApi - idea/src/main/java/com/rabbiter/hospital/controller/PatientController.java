...
public class PatientController {
   ...
   // public ResponseData login(@RequestBody Patient rpatient) {
    ...
    @ResponseBody
//    public ResponseData login(@RequestParam(value = "pId") int pId, @RequestParam(value = "pPassword") String pPassword) {
    public ResponseData loginwx(@RequestBody Patient rpatient) {
        Patient patient = this.patientService.login(rpatient.getPId(), rpatient.getPPassword());
        if (patient != null) {
            Map<String,String> map = new HashMap<>();
            map.put("pName", patient.getPName());
            map.put("pId", String.valueOf(patient.getPId()));
    ...
    public ResponseData patientDrug(){
        return  ResponseData.success("统计患者男女人数成功", this.patientService.patientDrug());

    }
    @RequestMapping("patientCheck")
    public ResponseData patientCheck(){
        return  ResponseData.success("统计患者男女人数成功", this.patientService.patientCheck());
